   const PLANILHA = "https://script.google.com/macros/s/AKfycbzpfOF8dbcM8Uzsh8qlqKBBzcKQbMPgNc_j7Ay7Y56xV7yGKa9RXjgxD0themalADsRIw/exec";  // Atualize com a URL do doGet
   const API_URL = "https://script.google.com/macros/s/AKfycbzpfOF8dbcM8Uzsh8qlqKBBzcKQbMPgNc_j7Ay7Y56xV7yGKa9RXjgxD0themalADsRIw/exec";
   ...
   function adicionarEntregueLocal(idUnico) {
     const entregues = getEntreguesLocais();
     entregues.add(idUnico);
   ...
   function excelSerialToDate(serial) {
     ...
     if (serial > 30000 && serial % 1 !== 0) return `${diaSemana}, ${d}/${m}/${y} – ${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}`;
     if (serial < 1) return `${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}`;
     return `${d}/${m}/${y}`;
   }
   ...
   async function carregarPlanilha() {
     try {
       const url = PLANILHA;  // Sem &t= ou fallback, pois o doGet é direto
   ...
   function renderizarCards(dados) {
     ...
     let pendentes = 0;
     let entregues = 0;
     const entreguesLocais = getEntreguesLocais();

     // Filtrar dados removendo itens marcados como entregues localmente
     const dadosFiltrados = dados.filter(linha => {
       ...
       return !entreguesLocais.has(idUnico);
     });
     ...
     document.getElementById("updateInfo").textContent = `Última atualização: ${agora.toLocaleTimeString()} — Atualizando automaticamente a cada 30 segundos`;
   }
